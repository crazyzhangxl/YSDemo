...
import com.videogo.ui.devicelist.DeviceModel;
    ...
    private void addQueryCameraAddVerifyCode() {
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    boolean result = MyApp.getOpenSDK().addDevice(mSerialNoStr, mVerifyCode);

                    /***********如有需要开发者需要自己保存此验证码***********/
//                    if (!TextUtils.isEmpty(mVerifyCode)) {
//                        //保存密码
//                        MyApp.getOpenSDK().setValidateCode(mVerifyCode, mSerialNoStr);
//                    }
