...
//    private UnbindDeviceTriggerHelper mTriggerHelper;
    ...
    private int probeDeviceInfo(String deviceSerial) {
        mEZProbeDeviceInfo = MyApp.getOpenSDK().probeDeviceInfo(serialNo,deviceType);
...
//        mTriggerHelper.onActivityResult(requestCode, resultCode, intent);
    ...
    private void start() {
       ...
       if(support_Wifi && support_sound_wave){
        ...
        MyApp.getOpenSDK().stopConfigWiFi();
        MyApp.getOpenSDK()
        ...
        new Thread() {
            public void run() {
                ...
                while (count > 0) {
                    ...
                    try {
                        MyApp.getOpenSDK().addDevice(serialNo, mVerifyCode);
...
//                        if (!TextUtils.isEmpty(mVerifyCode)) {
//                            //保存密码
//                            MyApp.getOpenSDK().setValidateCode(mVerifyCode, serialNo);
//                        }
